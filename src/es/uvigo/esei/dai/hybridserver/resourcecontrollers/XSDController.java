...
public class XSDController {
	...
	private XSDPages pages;
		
	public XSDController(XSDPages pages) {
		this.pages = pages;
	}
	...
	public HTTPResponse respondTo(HTTPRequest request) {
		...
		case GET:
			...
			if (request.getResourceParameters().containsKey("uuid")) {
				...
				contentBuilder.append("<h1>XSD</h1>");
		...
		// If it's POST
		case POST:
			String postContent = request.getResourceParameters().get("xsd");

			if (postContent == null) { // If there is no xsd content
				// Set HTTP response variables
				response.setVersion("HTTP/1.1");
			...
			response.setStatus(HTTPResponseStatus.S200); // 200 OK
			response.putParameter("Content-Type", MIME.TEXT_HTML.getMime());
			String linkToNewPage = "<a href=\"xsd?uuid=" + newUUID + "\">" + newUUID + "</a>";
